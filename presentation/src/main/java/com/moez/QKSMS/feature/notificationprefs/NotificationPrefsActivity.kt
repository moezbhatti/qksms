...
class NotificationPrefsActivity : QkThemedActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!router.hasRootController()) {
            val threadId = intent.extras?.getLong("threadId") ?: 0
            router.setRoot(RouterTransaction.with(NotificationPrefsController(threadId)))
        }
    }
