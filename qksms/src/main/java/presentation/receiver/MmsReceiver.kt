...
class MmsReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (Telephony.Sms.Intents.WAP_PUSH_DELIVER_ACTION == intent.action && ContentType.MMS_MESSAGE == intent.type) {
            ...
            when (pdu.messageType) {
                ...
                PduHeaders.MESSAGE_TYPE_READ_ORIG_IND -> {
                    ...
                    Timber.v("Received unhandled MMS type: ${pdu.messageType}")
                }
            }
        }
    }
