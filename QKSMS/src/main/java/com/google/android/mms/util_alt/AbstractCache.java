...
public abstract class AbstractCache<K, V> {
    ...
    public boolean put(K key, V value) {
        ...
        if (key != null) {
            ...
            if (LOCAL_LOGV) Log.v(TAG, key + " cached, " + mCacheMap.size() + " items total.");
            return true;
        }
        return false;
    }
    ...
    public V get(K key) {
        ...
        if (key != null) {
            ...
            if (cacheEntry != null) {
                cacheEntry.hit++;
                if (LOCAL_LOGV) Log.v(TAG, key + " hit " + cacheEntry.hit + " times.");
    ...
    public V purge(K key) {
        if (LOCAL_LOGV) Log.v(TAG, "Trying to purge " + key);

        CacheEntry<V> v = mCacheMap.remove(key);

        if (LOCAL_LOGV) Log.v(TAG, mCacheMap.size() + " items cached.");

        return v != null ? v.value : null;
    }

    public void purgeAll() {
        if (LOCAL_LOGV) Log.v(TAG, "Purging cache, " + mCacheMap.size()
                    + " items dropped.");
        mCacheMap.clear();
    }
