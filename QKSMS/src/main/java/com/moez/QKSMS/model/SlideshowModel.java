...
public class SlideshowModel extends Model
    ...
    public static SlideshowModel createFromPduBody(Context context, PduBody pb) throws MmsException {
        ...
        NodeList bodyNodes = docBody.getChildNodes();
        int bodyNum = bodyNodes.getLength();

        int mediaNum = 0;
        for (int i = 0; i < bodyNum; i++) {
            SMILParElement par = (SMILParElement) bodyNodes.item(i);
            NodeList mediaNodes = par.getChildNodes();
            int tempNum = mediaNodes.getLength();
        ...
        ArrayList<SlideModel> slides = new ArrayList<>(mediaNum);
        ...
        for (int i = 0; i < bodyNum; i++) {
            ...
            for (int j = 0; j < tempNum; j++) {
                SMILMediaElement sme = (SMILMediaElement) mediaNodes.item(j);
                ArrayList<MediaModel> mediaSet = new ArrayList<>(1);
                try {
                ...
                if(sme.getTagName().equals(SmilHelper.ELEMENT_TAG_TEXT)){
                    body = slide.getText();
                    continue;
                }
                else{
