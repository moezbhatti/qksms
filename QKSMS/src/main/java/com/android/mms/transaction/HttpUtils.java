...
public class HttpUtils {
    ...
    public static byte[] httpConnection(Context context, long token,
        ...
        if (LOCAL_LOGV) Log.v(TAG, "httpConnection: params list" +
                                   "\n\ttoken\t\t= " + token +
                                   "\n\turl\t\t= " + url +
                                   "\n\tmethod\t\t="
        ...
        try {
            ...
            req.addHeader(HDR_KEY_ACCEPT, HDR_VALUE_ACCEPT);
            {
                ...
                if (xWapProfileUrl != null) {
                    if (LOCAL_LOGV) Log.v(TAG, "[HttpUtils] httpConn: xWapProfUrl=" + xWapProfileUrl);
            ...
            if (entity != null) {
                try {
                    ...
                    if (entity.isChunked()) {
                        if (LOCAL_LOGV) Log.v(TAG, "httpConnection: transfer encoding is chunked");
                        ...
                        try {
                            ...
                            if (bytesRead == -1 && offset > 0 && !readError) {
                                ...
                                if (LOCAL_LOGV) Log.v(TAG, "httpConnection: Chunked response "
                                        + "length [" + Integer.toString(offset) + "]");
                            } else {
