...
public class PduParser {
    ...
    public GenericPdu parse(){
        ...
        if ((PduHeaders.MESSAGE_TYPE_SEND_REQ == messageType) ||
            ...
            // An M-Retrieve.conf PDU with the following headers from T-Mobile doesn't contain parts
            // in the body, but rather ASCII text "Message not found":
            //
            // DELIVERY_REPORT: VALUE_NO
            // PRIORITY: PRIORITY_NORMAL
            // DATE: Sat Jan 17 02:58:22 PST 1970
            // MMS_VERSION: MMS_VERSION_1_2
            // MESSAGE_CLASS: personal (TextString / byte[])
            // RETRIEVE_STATUS: RETRIEVE_STATUS_ERROR_PERMANENT_MESSAGE_NOT_FOUND
            // READ_REPORT: VALUE_NO
            // CONTENT_TYPE: text/plain (TextString / byte[])
            // RETRIEVE_TEXT: Message not found (EncodedStringValue)
            // MESSAGE_TYPE: MESSAGE_TYPE_RETRIEVE_CONF

            // Parsing parts from "Message not found" is an error, so check that there isn't a
            // retrieval error before parsing parts.
            int retrieveStatus = mHeaders.getOctet(PduHeaders.RETRIEVE_STATUS);
            if (!(retrieveStatus == PduHeaders.RETRIEVE_STATUS_ERROR_PERMANENT_FAILURE ||
                  retrieveStatus == PduHeaders.RETRIEVE_STATUS_ERROR_PERMANENT_CONTENT_UNSUPPORTED ||
                  retrieveStatus == PduHeaders.RETRIEVE_STATUS_ERROR_PERMANENT_MESSAGE_NOT_FOUND ||
                  retrieveStatus == PduHeaders.RETRIEVE_STATUS_ERROR_PERMANENT_SERVICE_DENIED)) {
        ...
        switch (messageType) {
            case PduHeaders.MESSAGE_TYPE_SEND_REQ:
                if (LOCAL_LOGV) Log.v(LOG_TAG, "parse: MESSAGE_TYPE_SEND_REQ");
                SendReq sendReq = new SendReq(mHeaders, mBody);
                return sendReq;
            case PduHeaders.MESSAGE_TYPE_SEND_CONF:
                if (LOCAL_LOGV) Log.v(LOG_TAG, "parse: MESSAGE_TYPE_SEND_CONF");
                SendConf sendConf = new SendConf(mHeaders);
                return sendConf;
            case PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND:
                if (LOCAL_LOGV) Log.v(LOG_TAG, "parse: MESSAGE_TYPE_NOTIFICATION_IND");
                NotificationInd notificationInd =
                    new NotificationInd(mHeaders);
                return notificationInd;
            case PduHeaders.MESSAGE_TYPE_NOTIFYRESP_IND:
                if (LOCAL_LOGV) Log.v(LOG_TAG, "parse: MESSAGE_TYPE_NOTIFYRESP_IND");
                NotifyRespInd notifyRespInd =
                    new NotifyRespInd(mHeaders);
                return notifyRespInd;
            case PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF:
                if (LOCAL_LOGV) Log.v(LOG_TAG, "parse: MESSAGE_TYPE_RETRIEVE_CONF");
            ...
            case PduHeaders.MESSAGE_TYPE_DELIVERY_IND:
                if (LOCAL_LOGV) Log.v(LOG_TAG, "parse: MESSAGE_TYPE_DELIVERY_IND");
                DeliveryInd deliveryInd =
                    new DeliveryInd(mHeaders);
                return deliveryInd;
            case PduHeaders.MESSAGE_TYPE_ACKNOWLEDGE_IND:
                if (LOCAL_LOGV) Log.v(LOG_TAG, "parse: MESSAGE_TYPE_ACKNOWLEDGE_IND");
                AcknowledgeInd acknowledgeInd =
                    new AcknowledgeInd(mHeaders);
                return acknowledgeInd;
            case PduHeaders.MESSAGE_TYPE_READ_ORIG_IND:
                if (LOCAL_LOGV) Log.v(LOG_TAG, "parse: MESSAGE_TYPE_READ_ORIG_IND");
                ReadOrigInd readOrigInd =
                    new ReadOrigInd(mHeaders);
                return readOrigInd;
            case PduHeaders.MESSAGE_TYPE_READ_REC_IND:
                if (LOCAL_LOGV) Log.v(LOG_TAG, "parse: MESSAGE_TYPE_READ_REC_IND");
    ...
    protected PduHeaders parseHeaders(ByteArrayInputStream pduDataStream){
        ...
        while (keepParsing && (pduDataStream.available() > 0)) {
            ...
            if ((headerField >= TEXT_MIN) && (headerField <= TEXT_MAX)) {
                ...
                if (LOCAL_LOGV) Log.v(LOG_TAG, "TextHeader: " + new String(bVal));
            ...
            switch (headerField) {
                case PduHeaders.MESSAGE_TYPE:
                {
                    ...
                    if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: messageType: " + messageType);
                    switch (messageType) {
                ...
                case PduHeaders.RESPONSE_STATUS:
                {
                    ...
                    if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: byte: " + headerField + " value: " +
                                value);
                ...
                case PduHeaders.MESSAGE_SIZE:
                {
                    try {
                        ...
                        if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: longint: " + headerField + " value: " +
                                    value);
                ...
                case PduHeaders.LIMIT:
                {
                    try {
                        ...
                        if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: int: " + headerField + " value: " +
                                    value);
                        headers.setLongInteger(value, headerField);
                    } catch(RuntimeException e) {
                        log(headerField + "is not Long-Integer header field!");
                ...
                case PduHeaders.CONTENT_LOCATION:
                {
                    ...
                    if (null != value) {
                        try {
                            if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: string: " + headerField + " value: " +
                                        new String(value));
                ...
                case PduHeaders.RESPONSE_TEXT:
                {
                    ...
                    if (null != value) {
                        try {
                            if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: encoded string: " + headerField
                ...
                case PduHeaders.TO:
                {
                    ...
                    if (null != value) {
                        ...
                        if (null != address) {
                            ...
                            if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: (to/cc/bcc) address: " + headerField
                                        + " value: " + str);
                ...
                case PduHeaders.REPLY_CHARGING_DEADLINE:
                {
                    ...
                    try {
                        if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: time value: " + headerField
                                    + " value: " + timeValue);
                ...
                case PduHeaders.FROM: {
                    ...
                    try {
                        if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: from address: " + headerField
                ...
                case PduHeaders.MESSAGE_CLASS: {
                    ...
                    if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: MESSAGE_CLASS: " + headerField
                                + " value: " + messageClass);
                ...
                case PduHeaders.MMS_VERSION: {
                    ...
                    try {
                        if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: MMS_VERSION: " + headerField
                                    + " value: " + version);
                ...
                case PduHeaders.PREVIOUSLY_SENT_BY: {
                    ...
                    if (null != previouslySentBy) {
                        try {
                            if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: PREVIOUSLY_SENT_BY: " + headerField
                ...
                case PduHeaders.PREVIOUSLY_SENT_DATE: {
                    ...
                    try {
                        ...
                        if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: PREVIOUSLY_SENT_DATE: " + headerField
                ...
                case PduHeaders.MM_FLAGS: {
                     ...
                     * Encoded-string-value
                     */
                    if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: MM_FLAGS: " + headerField
                ...
                {
                    if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: MBOX_TOTALS: " + headerField);
                    /* Value-length */
                    parseValueLength(pduDataStream);
                ...
                case PduHeaders.ELEMENT_DESCRIPTOR: {
                    if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: ELEMENT_DESCRIPTOR: " + headerField);
                ...
                case PduHeaders.CONTENT_TYPE: {
                    ...
                    if (null != contentType) {
                        try {
                            if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: CONTENT_TYPE: " + headerField +
                ...
                default: {
                    if (LOCAL_LOGV) Log.v(LOG_TAG, "parseHeaders: Unknown header: " + headerField);
                    log("Unknown header");
                }
            }
        }
    ...
    private static void log(String text) {
        if (LOCAL_LOGV) Log.v(LOG_TAG, text);
    }
        ...
        while(0 < lastLen) {
            ...
            switch (param) {
                ...
                case PduPart.P_NAME:
                    ...
                    break;
                default:
                    if (LOCAL_LOGV) Log.v(LOG_TAG, "Not supported Content-Type parameter");
...
//                                .android.internal.R.bool.config_mms_content_disposition_support);
                        ...
                        break;
                    default:
                        if (LOCAL_LOGV) Log.v(LOG_TAG, "Not supported Part headers: " + header);
                        if (-1 == skipWapValue(pduDataStream, lastLen)) {
