...
public class NotificationTransaction extends Transaction implements Runnable {
    ...
    public void run() {
        ...
        try {
            if (LOCAL_LOGV) Log.v(TAG, "Notification transaction launched: " + this);
            ...
            if (LOCAL_LOGV) Log.v(TAG, "Content-Location: " + mContentLocation);
            ...
            if (LOCAL_LOGV) Log.v(TAG, "status=0x" + Integer.toHexString(status));
